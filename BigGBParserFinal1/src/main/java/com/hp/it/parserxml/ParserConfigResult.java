...
public class ParserConfigResult {
	...
	private String numOfRDDLimit;
	
//	private String coalesceGenerateParquetNum;
	...
	public Map<String,Map<String,String>> initResultInMap(){
		...
		numOfRDDLimit=root.getChildText("numOfRDDLimit");
//		coalesceGenerateParquetNum=root.getChildText("coalesceGenerateParquetNum");
		...
		for(Element curDataEL : oneTypeOfTableList){
			...
			for(Element curEL : (List<Element>) curDataEL.getChildren("table")){
				...
				if(curEL.getAttributeValue("type").equals(typeOfTable)){
					...
					if(table.getKeyword()==null){
						tablesInfo.put(table.getTableName(),table);
					}else{
						tablesInfo.put(table.getKeyword(),table);
					}
				}
	...
	private Table createTable (Element tableEL,String schema ,String testTypeName){
		...
		for (Object element : tableEL.getChildren()) {
			...
			if(curEL.getName().equals("keyword")){
				table.setKeyword(curEL.getText());
			}
		}
...
//	public String getCoalesceGenerateParquetNum() {
//		return coalesceGenerateParquetNum;
//	}
//
//	public void setCoalesceGenerateParquetNum(String coalesceGenerateParquetNum) {
