...
public class RDDLoaderIntoDB implements VoidFunction<Iterator<Row>>{
	...
	private List<String> allTheParquetFilePath;
	
	public RDDLoaderIntoDB(Map<String, String> _stampValueMap, XmlSchema _xmlSchema , List<String> _masterStamps ,List<String> _allTheParquetFilePath) {
		super();
		this.stampValueMap = _stampValueMap;
		this.xmlSchema = _xmlSchema;
		this.masterStamps = _masterStamps;
		this.allTheParquetFilePath = _allTheParquetFilePath;
	}
	...
	public void call(Iterator<Row> rows) throws Exception {
		...
		//This value is to get this partition all the Tables
		...
		try{
			...
			while(rows.hasNext()){
				...
				if(!listCurrentPartitionTableNames.contains(tableName)){
					listCurrentPartitionTableNames.add(tableName);
				}
		...
		/**
		 * Because of one partition possible have all the some table data
...
//			for(String parquetFilePath:allTheParquetFilePath){
//				LoaderHelper.appendParquetFileDoneThatHaveLoadedIntoDB(listCurrentPartitionTableNames, parquetFilePath,hdfs);
//			}
		}catch(Exception e){
